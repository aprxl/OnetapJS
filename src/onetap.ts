...
export default class onetap {
  ...
  private static handle_errors(response: superagent.Response): boolean {
    ...
    logger.err(
      response.body.errors?.at(0).message ||
  ...
  public async get_config_by_name(name: string): Promise<config> {
    ...
    return new Promise((resolve) => {
      resolve(
        configs?.find(cfg => { return cfg.name == best_match })
      )
    });
  }
  ...
  public async get_script_by_name(name: string): Promise<script> {
    ...
    return new Promise((resolve) => {
      resolve(
        scripts?.find(cfg => { return cfg.name == best_match })
      )
    });
  }
  ...
  public async get_config_invite_by_name(name: string): Promise<config_invite[]> {
    const config = await this.get_config_by_name(name);
    const invites = await this.get_config_invites();

    return new Promise((resolve) => {
      resolve(
	invites?.filter(inv => { return inv.config_id === config.config_id })
      );
    })
  }

  public async create_config_invite(name: string, max_age = 0, max_uses = 0): Promise<config_invite> { 
    const config = await this.get_config_by_name(name);
    
    const response = await http.post(`configs/${config.config_id}/invites`, { 
  ...
  public async delete_config_invite(name: string, invite_id: number): Promise<boolean> { 
    const config = await this.get_config_by_name(name);

    const response = await http.delete(`configs/${config.config_id}/invites`, { 
  ...
  public async get_script_invite_by_name(name: string): Promise<script_invite[]> { 
    const script = await this.get_script_by_name(name);
    const invites = await this.get_script_invites();

    return new Promise((resolve) => { 
      resolve(
	invites?.filter(inv => { return inv.script_id === script.script_id })
      )
    })
  }

  public async create_script_invite(name: string, max_age = 0, max_uses = 0): Promise<script_invite> { 
    const script = await this.get_script_by_name(name);

    const response = await http.post(`scripts/${script.script_id}/invites`, { 
      'max_age': max_age.toString(),
      'max_uses': max_uses.toString()
    }, this.headers);
  ...
  public async delete_script_invite(name: string, invite_id: number): Promise<boolean> { 
    const script = await this.get_script_by_name(name);

    const response = await http.delete(`scripts/${script.script_id}/invites`, { 
      'invite_id': invite_id.toString()
    }, this.headers);
  ...
  public async get_config_subscription_by_name(name: string): Promise<config_subscription[]> { 
    const config = await this.get_config_by_name(name);

    const response = await http.get(`configs/${config.config_id}/subscriptions`, this.headers);
    const body = response.body;
  ...
  public async add_config_subscription(name: string, user_id: number): Promise<config_subscription> { 
    const config = await this.get_config_by_name(name);

    const response = await http.post(`configs/${config.config_id}/subscriptions`, { 
  ...
  public async remove_config_subscription(name: string, user_id: number): Promise<boolean> { 
    const config = await this.get_config_by_name(name);

    const response = await http.delete(`configs/${config.config_id}/subscriptions`, { 
      'user_id': user_id.toString()
    }, this.headers);
  ...
  public async get_script_subscription_by_name(name: string): Promise<script_subscription[]> { 
    const script = await this.get_script_by_name(name);
    const response = await http.get(`scripts/${script.script_id}/subscriptions`, this.headers);
  ...
  public async add_script_subscription(name: string, user_id: number): Promise<script_subscription> { 
    const script = await this.get_script_by_name(name);
    const response = await http.post(`scripts/${script.script_id}/subscriptions`, { 
      'user_id': user_id.toString()
    }, this.headers);
  ...
  public async remove_script_subscription(name: string, user_id: number): Promise<boolean> { 
    const script = await this.get_script_by_name(name);
    const response = await http.delete(`scripts/${script.script_id}/subscriptions`, { 
      'user_id': user_id.toString()
    }, this.headers);
